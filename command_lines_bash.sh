...
IN=/path/to/dataset1/file # filtered vcf file
OUT=/path/to/output/the/processed/files/folder
POPMAP=/path/to/popmap/file
...
finestructure -m T -x 100000 ${FILE}_chunks.out ${FILE}_chunks.mcmc.xml ${FILE}_chunks.mcmcTree.xml
		...
		### Dsuite ###
cd /path/to/folder/where/you/want/the/output		
FILE=/path/to/dataset1/file # vcf file
...
INPUT_3=/path/to/a filtered vcf file, that is snapp.vcf
STARTING_TREE=/path/to/a file with a starting tree, that is starting.tre
SPECIES=/path/to/a file with a table of samples and corresponding species/populations IDs, that is species.txt
CONSTRAINT=/path/to/a file specifying the age constraint, that is constraint.txt
...
# Run SNAPP
INPUT_SNAPP=/path/to/the xml file generated by the ruby script, that is dataset_3.xml
